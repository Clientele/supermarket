  ...
  <vs-sidebar click-not-close position-right parent="body" default-index="1" color="primary"
    ...
    <component :is="scrollbarTag" class="scroll-area--data-list-add-new" :settings="settings" :key="$vs.rtl">
      ...
      <div class="p-6">
        ...
        <!-- VENDOR NAME -->
        <vs-input label="Vendor Name" v-model="vendorInstance.vendor_name" class="mt-5 w-full" name="vendor_name" v-validate="'required'" />
        <span class="text-danger text-sm" v-show="errors.has('vendor_name')">{{ errors.first('vendor_name') }}</span>

        <!-- VENDOR EMAIL -->
        <vs-input label="Email" v-model="vendorInstance.email" class="mt-5 w-full" name="email" v-validate="'required'" />
        <span class="text-danger text-sm" v-show="errors.has('email')">{{ errors.first('email') }}</span>

        <!-- VENDOR PHONE 1-->
        <vs-input label="Phone Number 1" v-model="vendorInstance.phone_number_1" class="mt-5 w-full" name="phone_number_1" v-validate="'required'" />
        <span class="text-danger text-sm" v-show="errors.has('phone_number_1')">{{ errors.first('phone_number_1') }}</span>

        <!-- VENDOR PHONE 2 -->
        <vs-input label="Phone Number 2" v-model="vendorInstance.phone_number_2" class="mt-5 w-full" name="phone_number_2"  />
    ...
    <div class="flex flex-wrap items-center p-6" slot="footer">
      <vs-button class="mr-6" @click="saveVendor()" :disabled="!isFormValid">Submit</vs-button>
...
export default {
  ...
  methods: {
    ...
    saveVendor(){
      ...
      this.vendorInstance.country_id =  this.selectedAddress.country_id;
      this.vendorInstance.region_id =  this.selectedAddress.region_id;
